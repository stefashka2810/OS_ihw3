...
static void* room_thread(void*arg)
{
    ...
    if(w){
        ...
        pthread_t t; pthread_create(&t,NULL,room_thread,nr); pthread_detach(t);
        free(w);
    }
    pthread_mutex_unlock(&mtx);
    free(ra); return NULL;
...
static void* guest_thread(void*arg)
{
    ...
    if(r>=0){
        ...
        pthread_t t; pthread_create(&t,NULL,room_thread,ra); pthread_detach(t);
    }else{
...
int main(int argc,char*argv[])
{
    ...
    if(logger_init(argv[1], LOG_PORT)!=0){
        perror("logger_init"); return 1;
    }
    ...
    if(bind(l,(void*)&a,sizeof a)||listen(l,BACKLOG)){
    ...
    while(1){
        ...
        pthread_t t; pthread_create(&t,NULL,guest_thread,fd); pthread_detach(t);
    }
}
